    ...
    endpoints: (builder) => ({
            ...
            onQueryStarted: async ({product, quantity}, {dispatch, queryFulfilled}) => {
                ...
                const patchResult = dispatch(
                    basketApi.util.updateQueryData('fetchBasket', undefined, (draft) => {
                        ...
                        if (!isNewbasket) {
                            const exestingItems = draft.items.find(item => item.productId === productId);
                            if (exestingItems) exestingItems.quantity += quantity;
        ...
        removeBasketItem: builder.mutation<void, {productId: number, quantity: number}>({
            ...
            onQueryStarted: async ({productId, quantity}, {dispatch, queryFulfilled}) => {
                const patchResult = dispatch(
                    basketApi.util.updateQueryData('fetchBasket', undefined, (drafts) => {
                        const itemIndex = drafts.items.findIndex(item => item.productId == productId)

                        if (itemIndex >= 0) {
                            drafts.items[itemIndex].quantity -= quantity;
                            if (drafts.items[itemIndex].quantity <=0 ) {
                                drafts.items.splice(itemIndex, 1);
                            }
                        }
                    })
                )
