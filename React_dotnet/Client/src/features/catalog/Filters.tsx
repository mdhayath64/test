...
export default function Filters({filtersData: data}: Props) {
  ...
  return (
    <Box display='flex' flexDirection='column' gap={3}>
        ...
        <Paper sx={{p:3}}>
            <CheckboxButton checked={brands} items={data.brands} onChange={(items: string[]) => dispatch(setBrands(items))} />
        </Paper>
        <Paper sx={{p:3}}>
            <CheckboxButton checked={types} items={data.types} onChange={(items: string[]) => dispatch(setTypes(items))} />
        </Paper>
        <Button onClick={() => {
